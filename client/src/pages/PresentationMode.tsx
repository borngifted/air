...
export default function PresentationMode() {
  ...
  const scenes = useMemo<Scene[]>(() => [{ number: "00", kicker: "AiR · AI Readiness", title: "Clear the AiR. Then make something.", line: "Use the buttons, arrow keys, or your hand to show each teaching step.", notes: "Today, you are not just learning about AI. You are competing to see which team can communicate with it most effectively. Form four teams of five. Pick a captain and the car or the house.", prompt: "If AI is so smart, why doesn’t it always give us exactly what we want?", accent: "#d8ff45" }, ...catalog.flatMap(path => path.modules.flatMap(module => module.lessons.map(lesson => ({ number: lesson.number, kicker: `${path.title} · ${lesson.kicker}`, title: lesson.bigIdea, line: lesson.summary, notes: lesson.story, prompt: lesson.discussionPrompt, accent: path.accent }))))], [catalog]);
