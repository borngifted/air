...
export default function Dashboard() {
  return <PublicShell><AuthGate><DashboardContent /></AuthGate></PublicShell>;
}

function DashboardContent() {
  const { user } = useAuth();
  const { data, isLoading, error, refetch } = trpc.learning.dashboard.useQuery(undefined, { enabled: Boolean(user) });
  if (error) return <div className="container py-24"><QueryError message={error.message} retry={() => refetch()} /></div>;
  if (isLoading || !data) return <div className="container py-24"><div className="h-72 animate-pulse rounded-[2rem] bg-white/5" /></div>;
  ...
  return <>
    ...
    <section className="dashboard-body py-12 sm:py-16"><div className="container grid gap-6 lg:grid-cols-[1.35fr_.65fr]">
      <div className="dashboard-card lg:col-span-2"><ProjectList compact /></div>
      ...
      <div className="dashboard-card"><div className="flex items-center justify-between"><p className="eyebrow">Progress</p><CircleCheck className="size-5 text-[var(--go)]" /></div><div className="mt-8 flex items-end gap-2"><span className="display text-7xl">{completed}</span><span className="mb-3 text-xs uppercase tracking-[.12em] text-mist">of {lessons.length} lessons</span></div><Progress value={completion} className="mt-4 h-2" /><p className="mt-3 text-xs text-mist">Progress is private. It is here to help you resume—not compare.</p></div>
      <div className="dashboard-card lg:col-span-2"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div><p className="eyebrow">The four moves</p><h2 className="display mt-3 text-4xl">Move by move.</h2></div><Link href="/curriculum" className="text-link">See all lessons <ArrowRight className="size-4" /></Link></div><div className="mt-8 grid gap-3 md:grid-cols-2">{data.catalog.map(path => { const pathLessons = path.modules.flatMap(module => module.lessons); const done = pathLessons.filter(lesson => data.progress.some(item => item.lessonId === lesson.id && item.status === "completed")).length; return <Link key={path.id} href={`/paths/${path.slug}`} className="dashboard-path"><span className="path-number" style={{ color: path.accent }}>{path.number}</span><div><b>{path.title}</b><small>{done}/{pathLessons.length} lessons complete</small></div><ArrowRight className="ml-auto size-5" /></Link>; })}</div></div>
